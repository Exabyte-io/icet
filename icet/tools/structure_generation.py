...
def tmp(cluster_space, target_concentrations):
    ...
    for orbit in cluster_space.orbit_data:
        ...
        # weighted with their concentration
...
if __name__ == '__main__':
    ...
    #atoms.append(Atom('H', position=(2, 2, 2)))
    cs = ClusterSpace(atoms, [6.0], [['Au', 'Pd'], ['H', 'V']])

    maxsize = 10
    target_concentrations = {'Au': 3 / 10,
                             'Pd': 2 / 10,
                             'H': 4/10,
                             'V': 1/10,
                             }  # 'H': 1 / 4, 'V': 1 / 4}
    cv = tmp(cs, target_concentrations)
    print(cv)
    cv_random = get_random_vector(cs, target_concentrations)
    cv_random = cv_random[len(cv_random) - len(cv):]
    cv_diff = np.abs(cv) - np.abs(cv_random)
